...
For each time series, ARIMA models were fitted for all the combinations of $p$ and $q$ ranging from 1 to 7 and the model with the best Akaike information criterion (AIC) was chosen. The tools available in the package *forecast* in R [(2)] [references] were used for implementing the analysis, model fitting and forecast.

## Estimating delays in measure effects
Using a dynamic programming algorithm [(3)] [references] which estimates structural change points in regression models and implemented in the R package *strucchange* [(4)] [references], we estimated the lag time between measures taken by the Swiss government on the 17th of March and their effect on case and death numbers. Accordingly, each time series was split in an optimal number $m$ of segments, each segment of points being associated to a regression model. Using this methodology, we identified the time point corresponding to the beginning of the decreasing trend in the analysed time series.
...
(3) Bai, Jushan, and Pierre Perron. 2003. "Computation and Analysis of Multiple Structural Change Models.” Journal of Applied Econometrics 18 (1). Wiley Online Library: 1–22.

(4) Zeileis, Achim, Friedrich Leisch, Kurt Hornik, and Christian Kleiber. 2001. "strucchange. An R package for testing for structural change in linear regression models." Report Series SFB "Adaptive Information Systems and Modelling in Economics and Management Science", 55. 
